...
class DefaultImageEditorDelegate extends ImageEditorDelegate {
  ...
  Widget brushWidget(double limitSize, OperateType type,
    ...
    return Icon(Icons.brush_outlined,
        size: limitSize, color: operatorStatuscolor(choosen));
  }
  ...
  Widget rotateWidget(double limitSize, OperateType type,
      {required bool choosen}) {
    return Icon(Icons.rotate_right,
        size: limitSize, color: operatorStatuscolor(choosen));
  }

  @override
  Widget clipWidget(double limitSize, OperateType type,
      {required bool choosen}) {
    return Icon(
      Icons.cut,
      size: limitSize,
  ...
  Widget mosaicWidget(double limitSize, OperateType type,
      {required bool choosen}) {
    return Icon(Icons.auto_awesome_mosaic,
        size: limitSize, color: operatorStatuscolor(choosen));
  }
...
abstract class ImageEditorDelegate {
  final BoxConstraints _doneWidgetCons =
      BoxConstraints(maxHeight: 32, maxWidth: 54);
  ...
  Widget buildOperateWidget(OperateType type, {required bool choosen}) {
    switch (type) {
      ...
      case OperateType.clip:
        return clipWidget(_operateBtnSize, type, choosen: choosen);
